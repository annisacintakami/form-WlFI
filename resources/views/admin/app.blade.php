...
<head>
    ...
    <style>
        ...
        /* Munculkan overlay hanya saat class layout-menu-expanded aktif di tag <html> */
        .layout-menu-expanded .layout-overlay {
            display: block;
        }

        /* Pastikan sidebar berada di atas overlay */
        #layout-menu {
...
<body>
                ...
                <nav class="layout-navbar navbar navbar-expand-xl align-items-center">
                    ...
                    <form action="{{ route('contactadmin.index') }}" method="GET" class="search-box me-auto"
                        ...
                        <input type="text" name="search" id="searchInput" value="{{ request('search') }}"
                            class="form-control search-input" placeholder="Cari data..." autocomplete="off">
                    ...
                    <ul class="navbar-nav flex-row align-items-center">
                        ...
                        <li class="nav-item me-3 notif">
                            <a href="{{ route('contactadmin.index') }}" class="text-dark">
                                ...
                                @if(($newMessage ?? 0) > 0)
                                    <span
                                        class="badge bg-danger notif-badge position-absolute top-0 start-100 translate-middle">
                                        {{ $newMessage }}
                                    </span>
                                @endif
                            </a>
    ...
    <div id="layout-overlay" class="layout-overlay"></div>
    ...
    <script>
        ...
        $(document).ready(function () {
            // ... kode pencarian (search) yang sudah ada ...

            // --- LOGIKA TUTUP SIDEBAR ---
            const overlay = $('#layout-overlay');
            const htmlTag = $('html');

            // Fungsi untuk membuka sidebar (biasanya dipicu tombol menu)
            $('.layout-menu-toggle').on('click', function () {
                htmlTag.toggleClass('layout-menu-expanded');
            });

            // Fungsi saat area putih (overlay) diklik
            overlay.on('click', function () {
                htmlTag.removeClass('layout-menu-expanded');
            });
        });
    </script>
